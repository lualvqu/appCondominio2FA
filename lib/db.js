...
module.exports = function () { 
        ...
        global.db.collection("visitas").find(filtro).toArray( async function(err, results){
            if (!err){
                ...
                for (i = 0 ; i < results.length ; i++){
                    results[i].visitante = await global.db.collection("visitantes").findOne({_id: ObjectId(results[i].visitante_id) });
                }
            }
            callback(err, results);
        });
    ...
    const updateById = function (colletion, id, novosValores){
        global.db.collection(colletion).updateOne({_id:ObjectId(id)}, {$set:novosValores})
    }
